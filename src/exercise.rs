...
impl Exercise {
    ...
    pub fn expect_fixed_reps_mut(&mut self) -> (&mut ExerciseData, &mut FixedRepsExercise) {
        match self {
            Exercise::FixedReps(d, e) => (d, e),
            _ => panic!("expected fixed_reps"),
        }
    }

    pub fn expect_var_reps_mut(&mut self) -> (&mut ExerciseData, &mut VariableRepsExercise) {
        match self {
            Exercise::VariableReps(d, e) => (d, e),
            _ => panic!("expected var_reps"),
        }
    }
